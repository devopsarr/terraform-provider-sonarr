...
func (t resourceSeriesType) GetSchema(ctx context.Context) (tfsdk.Schema, diag.Diagnostics) {
	return tfsdk.Schema{
		...
		Attributes: map[string]tfsdk.Attribute{
			...
			/*TODO: waiting to implement these until empty conversion is managed natively https://www.terraform.io/plugin/framework/accessing-values#conversion-rules
			"seasons": {
				MarkdownDescription: "Seasons",
				Optional:            true,
				Attributes: tfsdk.SetNestedAttributes(map[string]tfsdk.Attribute{
					...
					"season_number": {
						MarkdownDescription: "Season Number",
						Computed:            true,
						Type:                types.Int64Type,
					},
			...
			"images": {
				...
				Attributes: tfsdk.SetNestedAttributes(map[string]tfsdk.Attribute{
					"cover_type": {
						MarkdownDescription: "Cover type",
						Required:            true,
						Type:                types.StringType,
					},
					"remote_url": {
						MarkdownDescription: "Full image URL",
						Required:            true,
						Type:                types.StringType,
					},
				}, tfsdk.SetNestedAttributesOptions{}),
...
func (r resourceSeries) Update(ctx context.Context, req tfsdk.UpdateResourceRequest, resp *tfsdk.UpdateResourceResponse) {
	...
	// Read again because PUT output is not reliable
	r.Read(ctx, tfsdk.ReadResourceRequest{
		State:        req.State,
		ProviderMeta: req.Config,
	}, (*tfsdk.ReadResourceResponse)(resp))
}
...
func readSeries(series *Series) *sonarr.AddSeriesInput {
	...
	return &sonarr.AddSeriesInput{
		...
		//		Images:            readImages(&series.Images),
		//		Seasons: 		   readSeasons(&series.Seasons),
		Tags: tags,
	}
}

/*
func readImages(images *[]Image) []*starr.Image {
	output := make([]*starr.Image, len(*images))

	for i, image := range *images {
		output[i] = &starr.Image{
			RemoteURL: image.RemoteURL.Value,
			CoverType: image.CoverType.Value,
		}
	}
...
func readSeasons(seasons *[]Season) []*sonarr.Season {
	output := make([]*sonarr.Season, len(*seasons))

	for i, s := range *seasons {
		output[i] = &sonarr.Season{
			SeasonNumber: int(s.SeasonNumber.Value),
...
func writeSeries(series *sonarr.Series) *Series {
	...
	//	genres := make([]types.String, len(series.Genres))
	//	for i, g := range series.Genres {
	//		genres[i] = types.String{Value: g}
	//	}

	//	seasons := writeSeasons(series.Seasons)
	//	images := writeImages(series.Images)
	return &Series{
...
/*
func writeSeasons(seasons []*sonarr.Season) *[]Season {
	output := make([]Season, len(seasons))
	for i, s := range seasons {
		output[i] = Season{
			Monitored:    types.Bool{Value: s.Monitored},
			SeasonNumber: types.Int64{Value: int64(s.SeasonNumber)},
		}
	}
	return &output
}
...
func writeImages(images []*starr.Image) *[]Image {
	output := make([]Image, len(images))
	for j, i := range images {
		output[j] = Image{
			CoverType: types.String{Value: i.CoverType},
			URL:       types.String{Value: i.URL},
			RemoteURL: types.String{Value: i.RemoteURL},
			Extension: types.String{Value: i.Extension},
		}
	}
	return &output
}
